...
class CourseReservesIndexer {
	...
	CourseReservesIndexer(String serverName, Ini configIni, Connection dbConn, Logger logger){
	...
	Long processPublicCourseReserves(boolean fullReindex, long lastReindexTime, CourseReservesIndexingLogEntry logEntry) {
		...
		try{
			...
			if (fullReindex){
				...
				numListsStmt = dbConn.prepareStatement("select count(id) as numLists from user_list WHERE deleted = 0 AND public = 1 and searchable = 1 AND isCourseReserve = 0");
				listsStmt = dbConn.prepareStatement("SELECT user_list.id as id, deleted, public, searchable, title, description, user_list.created, dateUpdated, username, firstname, lastname, displayName, homeLocationId, user_id from user_list INNER JOIN user on user_id = user.id WHERE public = 1 AND searchable = 1 AND deleted = 0 AND isCourseReserve = 0");
			}else{
				...
				numListsStmt = dbConn.prepareStatement("select count(id) as numLists from user_list WHERE isCourseReserve = 0");
				listsStmt = dbConn.prepareStatement("SELECT user_list.id as id, deleted, public, searchable, title, description, user_list.created, dateUpdated, username, firstname, lastname, displayName, homeLocationId, user_id from user_list INNER JOIN user on user_id = user.id WHERE dateUpdated > ? AND isCourseReserve = 0");
	...
	private boolean updateSolrForList(boolean fullReindex, ConcurrentUpdateSolrClient updateServer, PreparedStatement getTitlesForListStmt, ResultSet allPublicListsRS, long lastReindexTime, CourseReservesIndexingLogEntry logEntry) throws SQLException, SolrServerException, IOException {
		...
		if (!fullReindex && (deleted == 1 || isPublic == 0 || isSearchable == 0)){
			...
			CourseReserveSolr.setId(listId);
			CourseReserveSolr.setTitle(allPublicListsRS.getString("title"));
			CourseReserveSolr.setDescription(allPublicListsRS.getString("description"));
			...
			try {
				...
				if (userName.equalsIgnoreCase("nyt_user")) {
					CourseReserveSolr.setOwnerCanShareListsInSearchResults(true);
				}else{
					CourseReserveSolr.setOwnerCanShareListsInSearchResults(usersThatCanShareLists.contains(userId));
				}
				if (displayName != null && displayName.length() > 0){
					CourseReserveSolr.setAuthor(displayName);
				}else{
					...
					CourseReserveSolr.setAuthor(firstNameFirstChar + lastName);
				}

				long patronHomeLibrary = allPublicListsRS.getLong("homeLocationId");
				if (librariesByHomeLocation.containsKey(patronHomeLibrary)){
					CourseReserveSolr.setOwningLibrary(librariesByHomeLocation.get(patronHomeLibrary));
				} else {
					//Don't know the owning library for some reason, most likely this is an admin user.
					CourseReserveSolr.setOwningLibrary(-1);
				}

				//Don't know the owning location
				CourseReserveSolr.setOwningLocation(locationCodesByHomeLocation.getOrDefault(patronHomeLibrary, ""));
						...
						if (sourceId.length() > 0 && source.equals("GroupedWork")) {
							...
							try {
								...
								if (results.size() >= 1) {
									...
									CourseReserveSolr.addListTitle("grouped_work", sourceId, curWork.getFieldValue("title_display"), curWork.getFieldValue("author_display"));
								}
							} catch (Exception e) {
							...
							try {
								...
								if (results.size() >= 1) {
									SolrDocument curWork = results.get(0);
									CourseReserveSolr.addListTitle("open_archives", sourceId, curWork.getFieldValue("title"), curWork.getFieldValue("creator"));
								}
							} catch (Exception e) {
								logger.error("Error loading information about title " + sourceId);
							}
							...
							if (listDisplayNameAndAuthorRS.next()){
								...
								CourseReserveSolr.addListTitle("lists", sourceId, listDisplayNameAndAuthorRS.getString("title"), decryptedName);
							}
				...
				if (CourseReserveSolr.getNumTitles() >= 3) {
	...
	public long processCourseReserves(boolean fullReindex, long lastReindexTime, CourseReservesIndexingLogEntry logEntry) {
		...
		try{
			...
			if (fullReindex){
				...
				updateServer.deleteByQuery("recordtype:list");
				//Get a list of all public lists
				numCourseReservesStmt = dbConn.prepareStatement("select count(id) as numLists from user_list WHERE deleted = 0 AND public = 1 and searchable = 1 AND isCourseReserve = 1");
				courseReservesStmt = dbConn.prepareStatement("SELECT user_list.id as id, deleted, public, searchable, title, description, user_list.created, dateUpdated, courseInstructor, courseNumber, courseTitle, courseLibrary from user_list WHERE public = 1 AND searchable = 1 AND deleted = 0 AND isCourseReserve = 1");
			}else{
				//Get a list of all lists that are were changed since the last update
				//Have to process all lists because one could have been deleted, made private, or made non searchable.
				numCourseReservesStmt = dbConn.prepareStatement("select count(id) as numLists from user_list WHERE isCourseReserve = 1");
				courseReservesStmt = dbConn.prepareStatement("SELECT user_list.id as id, deleted, public, searchable, title, description, user_list.created, dateUpdated, courseInstructor, courseNumber, courseTitle, courseLibrary from user_list WHERE dateUpdated > ? AND isCourseReserve = 1");
