						...
						<div class="btn-toolbar">
							<div class="btn-group btn-group-vertical btn-block">
								...
								{foreach from=$summaryActions item=link}
									<a href="{if $link.url}{$link.url}{else}#{/if}" {if $link.onclick && strlen($link.onclick) > 0}onclick="{$link.onclick}"{/if} class="btn btn-sm btn-action" target="_blank">{$link.title}</a>&nbsp;
								{/foreach}
							</div>
						</div>
					</div>
