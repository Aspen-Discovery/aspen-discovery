...
my $marcout_command_line_no_file = "$marcout_exe /s$hz_server/p$hz_password/u$hz_user/d$hz_database/tbibs_to_export/xitem949/y/q999/b1/e0/\@m";
...
MAINLOOP: while (1 == 1) {
  ...
  my $dbh = DBI->connect("DBI:ODBC:Driver={SQL Server};Server=$hz_server;maxConnect=100;UID=$hz_user;PWD=$hz_password");
   ...
   if (0 == 1) {
    ...
    eval {
      $get_pending = $dbh->selectall_arrayref("
        select top $max_records_per_cycle SerialNum,RecNum,RecType,Action from solr_pending
        order by SerialNum
      ");
    };
    if ($@) {
    ...
    if (@$get_pending) {
      ...
      if (%new_and_updates) {
        ...
        # clear the table bibs_to_export
        eval {
          $dbh->do("truncate table bibs_to_export");
        };
        if ($@) {
          # drop the connection
          undef($dbh);
        ...
        foreach my $bid (keys(%new_and_updates)) {
          $n_exported++;
          # put the bib# in the bibs_to_export table
