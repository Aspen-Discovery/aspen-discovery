{strip}
<div id="record{if !empty($summShortId)}{$summShortId}{else}{$summId|escape}{/if}" class="resultsList-summon resultsList row">
	{if !empty($showCovers)}
	...
	<div class="{if empty($showCovers)}col-xs-12{else}col-tn-9 col-sm-9{if empty($viewingCombinedResults)} col-md-9 col-lg-10{/if}{/if}">
		...
		{if strlen($summSourceDatabase)}
			<div class="row hidden-phone result-source-database">
		...
		{if !empty($summPhysical)}
			<div class="row hidden-phone result-physical-description">
		...
		<div class="row hidden-phone result-full-text">
		...
		{if !empty($summDescription)}
			...
			<div class="row visible-xs result-description">
