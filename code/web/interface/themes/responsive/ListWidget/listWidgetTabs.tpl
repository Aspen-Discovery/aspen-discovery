			...
			{foreach from=$widget->lists item=list name=listLoop}
				...
				{if $list->displayFor == 'all' || ($list->displayFor == 'loggedIn' && $loggedIn) || ($list->displayFor == 'notLoggedIn' && !$loggedIn)}
					{if $index == 0}
						...
						listScroller{$listName}.loadTitlesFrom('/Search/AJAX?method=getSpotlightTitles%26id={$list->id}%26scrollerName={$listName}%26coverSize={$widget->coverSize}%26showRatings={$widget->showRatings}%26numTitlesToShow={$widget->numTitlesToShow}{if $reload}%26reload=true{/if}', false);
					{/if}
