	...
	<div class="facet-form">
		...
		{if $title == 'publishDate' || $title == 'publishDateSort'}
			<div id='yearDefaultLinks'>
				...
				<a onclick="$('#{$title}yearfrom').val('{$thisyear-1}');$('#{$title}yearto').val('');" href='javascript:void(0);'>{translate text="year"}</a>
				&bullet; <a onclick="$('#{$title}yearfrom').val('{$thisyear-5}');$('#{$title}yearto').val('');" href='javascript:void(0);'>{translate text="5 years"}</a>
				&bullet; <a onclick="$('#{$title}yearfrom').val('{$thisyear-10}');$('#{$title}yearto').val('');" href='javascript:void(0);'>{translate text="10 years"}</a>
			</div>
		{/if}
	</div>
</form>
