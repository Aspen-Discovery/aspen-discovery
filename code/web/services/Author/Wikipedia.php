...
class Author_Wikipedia {
	...
	public function getWikipedia($author, $lang = 'en') {
		...
		$url = "http://{$lang}.wikipedia.org/w/api.php" . '?action=query&prop=revisions&rvprop=content&format=json' . '&titles=' . urlencode($author);
		...
		if ($result == null) {
			...
			if (strpos($author, ',') > 0) {
				...
				$url = "http://{$lang}.wikipedia.org/w/api.php" . '?action=query&prop=revisions&rvprop=content&format=json' . '&titles=' . urlencode($tmpAuthor);
