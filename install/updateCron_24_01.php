...
if (count($_SERVER['argv']) > 1) {
	...
	if ($fhnd) {
		...
		while (($line = fgets($fhnd)) !== false) {
			if (strpos($line, 'cleanupSharedSessions') > 0) {
		...
		if ($insertUpdateTranslations) {
			$lines[] = "######################################\n";
			$lines[] = "# Cleanup Shared Sessions #\n";
			$lines[] = "######################################\n";
			$lines[] = "15 * * * * root php /usr/local/aspen-discovery/code/web/cron/cleanupSharedSessions.php $serverName\n";
		}
		if ($insertUpdateTranslations) {
