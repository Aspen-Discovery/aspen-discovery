...
<div class="controls table-responsive">
	...
	<div class="{$propName}Actions">
		...
		{/if}
		{if !empty($property.additionalOneToManyActions) && (!empty($id) || !empty($property.newCanDoAdditionalActions))}{* Only display these actions for an existing object *}
			<div class="btn-group pull-right" style="padding-top: 0">
				{foreach from=$property.additionalOneToManyActions item=action}
					<a class="btn {if !empty($action.class)}{$action.class}{else}btn-primary{/if} btn-sm" {if !empty($action.url)}href="{$action.url|replace:'$id':$id|default:''}"{/if} {if !empty($action.onclick)}onclick="{$action.onclick|replace:'$id':$id|default:''}"{/if}>{translate text=$action.text isAdminFacing=true}</a>
				{/foreach}
			</div>
		{/if}
	</div>
