...
import {getLists, PATRON} from '../../../util/loadPatron';
...
export default class MyLists extends Component {
	...
	async componentDidUpdate(prevProps, prevState) {
		...
		if (prevState.listChanges !== route.params?.hasPendingChanges) {
			this.setState({
				lists: PATRON.lists,
			});
			navigation.dispatch({
	...
	renderList = (item, libraryUrl) => {
		...
		if (item.id !== 'recommendations') {
			return (
					...
					}} borderBottomWidth="1" _dark={{borderColor: 'gray.600'}} borderColor="coolGray.200" pl="1" pr="1" py="2">
