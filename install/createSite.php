...
if (count($_SERVER['argv']) > 1){
	...
	if (file_exists($siteConfigFile)){
		...
		$variables = [
			'sitename' => $sitename,
            'configDir' => $configArray['Site']['configDir'] ?? "/usr/local/aspen-discovery/sites/$sitename",
...
if (!$foundConfig) {
    ...
    $variables['configDir'] = readline("Enter the directory where the configurations will be stored (by default /usr/local/aspen-discovery/sites/$sitename)?");
    if (empty($variables['configDir'])){
        $variables['configDir'] = "/usr/local/aspen-discovery/sites/$sitename";
    }
	...
	if (empty($variables['databasePort'])){
		$variables['databasePortPort'] = "3306";
	}
