...
<input type='password' name='{$propName}Repeat' id='{$propName}Repeat' {if {$propValue_Repeat}value='{$propValue_Repeat|escape}'{/if} {if $property.maxLength}maxlength='{$property.maxLength}'{/if} {if !empty($property.size)}size='{$property.size}'{/if} class='form-control repeat' {if !empty($property.readOnly)}readonly{/if} />
{/if}