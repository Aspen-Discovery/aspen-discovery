    ...
    <Center>
      ...
      >
        <Modal.Content maxWidth="95%">
          <Modal.CloseButton />
          <Modal.Header>
            {translate("linked_accounts.account_to_manage")}
          </Modal.Header>
          <Modal.Body>
            <FormControl>
              <FormControl.Label>
                {translate("linked_accounts.username")}
              </FormControl.Label>
              <Input
            ...
            <FormControl mt={3}>
              <FormControl.Label>
                {translate("linked_accounts.password")}
              </FormControl.Label>
              <Input
          ...
          <Modal.Footer>
            <Button.Group>
              ...
              <Button
                ...
                onPress={async () => {
                  setLoading(true);
                  await addLinkedAccount(user, password, props.libraryUrl).then(
                    (r) => {
                      setShowModal(false);
                      setLoading(false);
