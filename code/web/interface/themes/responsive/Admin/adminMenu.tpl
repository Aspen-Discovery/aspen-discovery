{strip}
    ...
    <a class='searchSettings searchSettingsColor' onClick="return AspenDiscovery.Admin.showSearch();" id='showSearchButton'><i class="fas fa-search" ></i> {translate text="Search" isAdminFacing=true}</a>
  </div>
    <form id='adminSearchBox' role="form" style='display:none'>
    <div class="form-group">
      <label class='searchSettings' for="settingsSearch">{translate text="Search for a Setting" isAdminFacing=true}</label>
      <div class="input-group">
        ...
        <span class="input-group-btn"><button class="btn btn-default" type="button" onclick="$('#searchAdminBar').val('');return AspenDiscovery.Admin.searchAdminBar();" title="{translate text="Clear" inAttribute=true isAdminFacing=true}"><i class="fas fa-times-circle"></i></button></span>
      ...
      {foreach from=$adminActions item=adminSection key=adminSectionKey}
        {if !empty($adminSection->actions)}
          <div class="admin-menu-section panel{if $adminSectionKey==$activeAdminSection} active{/if}">
            ...
            <div id="{$adminSectionKey}Group" class="panel-collapse collapse admin-search-collapse {if $adminSectionKey==$activeAdminSection}in{/if}">
              <div class="panel-body">
									{foreach from=$adminSection->actions item=adminAction key=adminActionKey}
										<div class="adminMenuLink "><a class='adminLink' href="{$adminAction->link}">{translate text=$adminAction->label isAdminFacing=true}</a></div>
