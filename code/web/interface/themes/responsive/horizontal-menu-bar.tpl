{strip}
	...
	<div class="menu-section menu-section-left">
		{if !empty($homeLink)}
			<a href="{$homeLink}" id="homeLink" class="menu-icon menu-bar-option" title="{translate text='Library Home Page'}" aria-label="{translate text="Return to $homeLinkText"}">
				<i class="fas fa-home fa-lg"></i><span class="menu-bar-label hidden-inline-block-xs"></span>
			</a>
		{/if}
		<a href="/Search/Home" id="homeLink" class="menu-icon menu-bar-option" title="{translate text='Browse the catalog'}" aria-label="{translate text='Browse the catalog'}">
		...
		{foreach from=$libraryLinks key=categoryName item=menuCategory}
			...
			{if $topCategory->showInTopMenu || $topCategory->alwaysShowIconInTopMenu}
				{if count($menuCategory) > 1}
					<a id="{$topCategory->getEscapedCategory()}-menu-trigger" onclick="return AspenDiscovery.showCustomMenu('{$topCategory->getEscapedCategory()}')" class="menu-icon menu-bar-option {if !$topCategory->alwaysShowIconInTopMenu}visible-inline-block-lg{/if}" title="{translate text=$categoryName}" aria-label="{translate text=$categoryName}">
				...
				{else}
					<a href="{$topCategory->url}" class="menu-icon menu-bar-option {if !$topCategory->alwaysShowIconInTopMenu}visible-inline-block-lg{/if}" title="{translate text=$categoryName}" aria-label="{translate text=$categoryName}">
						{if !empty($topCategory->iconName)}
							<i class="fas fa-{$topCategory->iconName} fa-lg"></i>
						{/if}
						<span class="menu-bar-label visible-inline-block-lg">{$topCategory->category}</span>
	...
	<div class="menu-section menu-section-right">
		...
		<a id="menuToggleButton" onclick="return AspenDiscovery.toggleMenu();" class="menu-icon menu-bar-option" title="{translate text="Show Menu"}">
