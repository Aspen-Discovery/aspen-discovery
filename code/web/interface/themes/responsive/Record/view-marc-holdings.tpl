{strip}
	...
	{if count($otherMarcHoldings) > 0}
		{if count($localMarcHoldings) > 0}<button onclick="$('#otherMarcHoldings').show();$('#showOtherMarcHoldings').hide();return false;" id="showOtherMarcHoldings" >{translate text="Show other holdings" isPublicFacing=true}</button>{/if}
		<div id="otherMarcHoldings" {if count($localMarcHoldings) > 0}style="display: none"{/if}>
			{if count($localMarcHoldings) > 0}<h3>{translate text="Other Holdings" isPublicFacing=true}</a></h3>{/if}
