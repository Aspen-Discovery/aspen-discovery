		...
		<div class="row">
					...
					<div class="btn-group btn-group-vertical btn-block">
						...
						{else}
							{if !empty($recordDriver->getRegistrationModalBody())}
								<a class="btn btn-sm btn-action btn-register btn-wrap" onclick="return AspenDiscovery.Account.regInfoModal(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'localhop', '{$recordDriver->getExternalUrl()}');" style="width:70%">{translate text="Registration Information" isPublicFacing=true}
								</a>
							{else}
			...
			{else}
				{if $recordDriver->isRegistrationRequired()}
					<div class="btn-group btn-group-vertical btn-block">
						...
						{if empty($offline) || $enableEContentWhileOffline}
							<a onclick="return AspenDiscovery.Account.saveEvent(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'localhop');"
				...
				{elseif empty($offline) || $enableEContentWhileOffline}
					<a class="btn btn-sm btn-action btn-wrap addToYourEventsBtn" style="width:70%" onclick="return AspenDiscovery.Account.saveEvent(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'localhop');">{translate text="Add to Your Events" isPublicFacing=true}</a>
				{/if}
			{/if}
		</div>
		</div>
...
{*Staff View Div*}
{if !empty($loggedIn) && (in_array('Administer LocalHop Settings', $userPermissions))}
	<div class="row">
		<div id="more-details-accordion" class="panel-group">
			<div class="panel" id="staffPanel">
				...
				<div id="staffPanelBody" class="panel-collapse collapse">
					<div class="panel-body">
						<h3>{translate text="LocalHop Event API response" isPublicFacing=true}</h3>
