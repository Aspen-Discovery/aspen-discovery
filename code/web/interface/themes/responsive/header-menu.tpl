						...
						{foreach from=$linkCategory item=link key=linkName}
							...
							{else}
								<div class="header-menu-option {if !empty($categoryName) && !preg_match('/none-\\d+/', $categoryName)}childMenuItem{/if}">
									<a href="{$link->url}" {if $link->openInNewTab}target="_blank"{/if} aria-label="{translate text=$linkName inAttribute=true isPublicFacing=true}{if $link->openInNewTab} ({translate text="opens in a new window" isPublicFacing=true inAttribute=true}){/if}" title="{translate text=$linkName inAttribute=true isPublicFacing=true}{if $link->openInNewTab} ({translate text="opens in a new window" isPublicFacing=true inAttribute=true}){/if}">
