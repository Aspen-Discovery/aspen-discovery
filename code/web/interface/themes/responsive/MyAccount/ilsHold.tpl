					...
					<div class="btn-group btn-group-vertical btn-block">
						{if $section == 'available'}
							...
							{if $record->cancelable}
								...
								<button onclick="return AspenDiscovery.Account.confirmCancelHold('{$record->userId}', '{$record->sourceId}', '{$record->cancelId}', {$record->isIll});" class="btn btn-sm btn-warning">{translate text="Cancel Hold" isPublicFacing=true}</button>
							{/if}
						{else}
							{if $record->cancelable}
								{* First step in cancelling a hold is now fetching confirmation message, with better labeled buttons. *}
