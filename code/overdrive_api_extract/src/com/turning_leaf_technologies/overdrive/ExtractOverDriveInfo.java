...
class ExtractOverDriveInfo {
	...
	void extractOverDriveInfo(Ini configIni, Connection dbConn, OverDriveExtractLogEntry logEntry, boolean doFullReload, String individualIdToProcess) {
		this.dbConn = dbConn;
		...
		try {
			addProductStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_products set overdriveid = ?, crossRefId = ?, mediaType = ?, title = ?, subtitle = ?, series = ?, primaryCreatorRole = ?, primaryCreatorName = ?, cover = ?, dateAdded = ?, dateUpdated = ?, lastMetadataCheck = 0, lastMetadataChange = 0, lastAvailabilityCheck = 0, lastAvailabilityChange = 0, rawData=?", PreparedStatement.RETURN_GENERATED_KEYS);
			setNeedsUpdateStmt = dbConn.prepareStatement("UPDATE overdrive_api_products set needsUpdate = ? where overdriveid = ?");
			...
			getNumProductsNeedingUpdatesStmt = dbConn.prepareCall("SELECT count(overdrive_api_products.id) from overdrive_api_products where needsUpdate = 1 and deleted = 0 LIMIT " + maxProductsToUpdate);
			getProductsNeedingUpdatesStmt = dbConn.prepareCall("SELECT overdrive_api_products.id, overdriveId, crossRefId, lastMetadataCheck, lastMetadataChange, lastAvailabilityCheck, lastAvailabilityChange from overdrive_api_products where needsUpdate = 1 and deleted = 0 LIMIT " + maxProductsToUpdate);
			getIndividualProductStmt = dbConn.prepareCall("SELECT overdrive_api_products.id, overdriveId, crossRefId, lastMetadataCheck, lastMetadataChange, lastAvailabilityCheck, lastAvailabilityChange from overdrive_api_products WHERE overdriveId = ?");
			updateProductStmt = dbConn.prepareStatement("UPDATE overdrive_api_products SET crossRefId = ?, mediaType = ?, title = ?, subtitle = ?, series = ?, primaryCreatorRole = ?, primaryCreatorName = ?, cover = ?, dateUpdated = ?, deleted = 0, rawData=? where id = ?");
			...
			loadMetaDataStmt = dbConn.prepareStatement("SELECT * FROM overdrive_api_product_metadata WHERE productId = ?");
			updateMetaDataStmt = dbConn.prepareStatement("UPDATE overdrive_api_product_metadata set productId = ?, checksum = ?, sortTitle = ?, publisher = ?, publishDate = ?, isPublicDomain = ?, isPublicPerformanceAllowed = ?, shortDescription = ?, fullDescription = ?, starRating = ?, popularity =?, thumbnail=?, cover=?, isOwnedByCollections=?, rawData=? where id = ?");
			addMetaDataStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_metadata set productId = ?, checksum = ?, sortTitle = ?, publisher = ?, publishDate = ?, isPublicDomain = ?, isPublicPerformanceAllowed = ?, shortDescription = ?, fullDescription = ?, starRating = ?, popularity =?, thumbnail=?, cover=?, isOwnedByCollections=?, rawData=?");
			clearCreatorsStmt = dbConn.prepareStatement("DELETE FROM overdrive_api_product_creators WHERE productId = ?");
			addCreatorStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_creators SET productId = ?, role = ?, name = ?, fileAs = ?");
			PreparedStatement loadLanguagesStmt = dbConn.prepareStatement("SELECT * FROM overdrive_api_product_languages");
			addLanguageStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_languages set code =?, name = ?", PreparedStatement.RETURN_GENERATED_KEYS);
			clearLanguageRefStmt = dbConn.prepareStatement("DELETE FROM overdrive_api_product_languages_ref where productId = ?");
			addLanguageRefStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_languages_ref set productId = ?, languageId = ?");
			PreparedStatement loadSubjectsStmt = dbConn.prepareStatement("SELECT * FROM overdrive_api_product_subjects");
			addSubjectStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_subjects set name = ?", PreparedStatement.RETURN_GENERATED_KEYS);
			clearSubjectRefStmt = dbConn.prepareStatement("DELETE FROM overdrive_api_product_subjects_ref where productId = ?");
			addSubjectRefStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_subjects_ref set productId = ?, subjectId = ?");
			...
			updateAvailabilityStmt = dbConn.prepareStatement("UPDATE overdrive_api_product_availability set available = ?, copiesOwned = ?, copiesAvailable = ?, numberOfHolds = ?, availabilityType = ? WHERE id = ?");
			addAvailabilityStmt = dbConn.prepareStatement("INSERT INTO overdrive_api_product_availability set productId = ?, libraryId = ?, available = ?, copiesOwned = ?, copiesAvailable = ?, numberOfHolds = ?, availabilityType = ?");
			...
			markGroupedWorkForBibAsChangedStmt = dbConn.prepareStatement("UPDATE grouped_work SET date_updated = ? where id = (SELECT grouped_work_id from grouped_work_primary_identifiers WHERE type = 'overdrive' and identifier = ?)") ;
			...
			if (individualIdToProcess != null) {
				...
				try{
					PreparedStatement loadPartialExtractRunning = dbConn.prepareStatement("SELECT * from variables WHERE name = 'partial_overdrive_extract_running'");
	...
	private void addProductToDB(OverDriveRecordInfo overDriveInfo) throws SocketTimeoutException {
		...
		try {
			...
			updateOverDriveMetaData(overDriveInfo, databaseId);
			updateOverDriveAvailability(overDriveInfo, databaseId);
		} catch (SQLException e) {
	...
	private boolean loadProductsFromDatabase() {
		try {
			...
			while (loadProductsRS.next()){
				...
				curProduct.setSubtitle(loadProductsRS.getString("subtitle"));
								...
								if (advantageWebServiceResponse.getResponseCode() == 200) {
									...
									if (advantageSelfInfo != null) {
										...
										if (advantageLibraryId != -1L){
											...
											logger.info("Skipping advantage account " + advantageName + " because it does not have an associated library");
										}
	...
	private void updateOverDriveMetaData(OverDriveRecordInfo overDriveInfo, long databaseId) throws SocketTimeoutException {
	...
	private void updateOverDriveMetaDataBatch(long libraryId, List<MetaAvailUpdateData> productsToUpdateBatch) throws SocketTimeoutException {
		...
		if (metaDataResponse.getResponseCode() != 200){
			...
			logger.error("Error " + metaDataResponse.getResponseCode() + " retrieving batch metadata for batch " + url + " " + metaDataResponse.getMessage());
		}else{
			JSONObject bulkResponse = metaDataResponse.getJSONResponse();
								...
								if (metadata.getBoolean("isOwnedByCollections")){
									...
									for (String advantageKey : libToOverDriveAPIKeyMap.values()){
										...
										if (advantageMetaDataResponse.getResponseCode() != 200){
											//Doesn't exist in this collection, skip to the next.
											logger.error("Error " + advantageMetaDataResponse.getResponseCode() + " retrieving metadata for advantage account " + url + " " + metaDataResponse.getMessage());
										}else {
											JSONObject advantageMetadata = advantageMetaDataResponse.getJSONResponse();
	...
	private void updatePartialExtractRunning(boolean running) {
		...
		try {
			if (partialExtractRunningVariableId != null) {
				...
				PreparedStatement insertVariableStmt = dbConn.prepareStatement("INSERT INTO variables (`name`, `value`) VALUES ('partial_overdrive_extract_running', ?)", Statement.RETURN_GENERATED_KEYS);
