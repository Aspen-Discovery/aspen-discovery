...
export default class Settings_HomeScreen extends Component {
  ...
  updateToggle = async (item, user, libraryUrl) => {
    if (item.isHidden === true) {
      this._isMounted &&
        (await showBrowseCategory(
          libraryUrl,
          item.key,
          user,
          this.state.discoveryVersion
        ).then(async (res) => {
    ...
    return (
      ...
      >
        <HStack
          ...
          pb={1}
        >
          <Text isTruncated bold maxW="80%" fontSize={{ base: "lg", lg: "xl" }}>
            {item.title}
          </Text>
          {item.isHidden ? (
            <Switch
              size="md"
              onToggle={() =>
                this.updateToggle(item, patronId, libraryUrl, browseCategories)
              }
            />
          ) : (
            <Switch
