								...
								<td class="myAccountCell">
									{if ($event.regRequired == 1)}
										{if $event.externalLink != null && !($event.isRegistered)}
											...
											{else}
												<a href="{$event.externalLink}" class="btn btn-xs btn-action" target="_blank"><i class="fas fa-external-link-alt" role="presentation"></i> {translate text="Check Registration" isPublicFacing=true}</a>
											{/if}
										{elseif $event.isRegistered}
											<a href="{$event.externalLink}" class="btn btn-xs btn-action" target="_blank"><i class="fas fa-external-link-alt" role="presentation"></i> {translate text=" You Are Registered" isPublicFacing=true}</a>
										{else}
