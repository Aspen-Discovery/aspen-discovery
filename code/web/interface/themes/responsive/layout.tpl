...
<head prefix="og: http://ogp.me/ns#">
	{strip}
		...
		{/if}
		{if !empty($themeCss)}{$themeCss}{if !empty($userFontSizeOverride)}<style>:root{ldelim}--aspen-base-font-size: {$userFontSizeOverride};{rdelim}</style>{/if}{/if}
		{if $isRTL && !empty($themeRTLCss)}{$themeRTLCss}{if !empty($userFontSizeOverride)}<style>:root{ldelim}--aspen-base-font-size: {$userFontSizeOverride};{rdelim}</style>{/if}{/if}
