...
public class AnselToUnicode extends CharConverter {
    ...
    private CodeTableInterface ct;

    private boolean loadedMultibyte = false;
    ...
    public String convert(final char data[]) {
        ...
        while (cdt.offset < data.length) {
            if (ct.isCombining(data[cdt.offset], cdt.g0, cdt.g1) && hasNext(cdt.offset, len)) {
                while (cdt.offset < len && ct.isCombining(data[cdt.offset], cdt.g0, cdt.g1) && hasNext(cdt.offset, len)) {
        ...
        if (translateNCR && conversionPattern1.matcher(dataElement).matches()) {
            final Matcher matcher = conversionPattern2.matcher(dataElement);
    ...
    private String convertMultibyte(final CodeTracker cdt, final char[] data) {
        ...
        while (offset < data.length && data[offset] != 0x1b) {
            ...
            if (data[offset] == 0x20) {
                ...
                cdt.g1 = 0x45;
                break;
            } else if (offset + 3 <= data.length && getMBChar(makeMultibyte(data[offset], data[offset + 1], data[offset + 2])) != 0) {
                final char c = getMBChar(makeMultibyte(data[offset], data[offset + 1], data[offset + 2]));
                if (curReader == null || c != 0) {
                    sb.append(c);
                    offset += 3;
     ...
     */
    private String getMBCharStr(final int ch) {
