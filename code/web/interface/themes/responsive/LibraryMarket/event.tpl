					...
					{if $recordDriver->isRegistrationRequired()}
						<div class="btn btn-sm btn-action btn-wrap" style="width:70%">
							{if !empty($recordDriver->getRegistrationModalBody())}
								<a class="btn btn-sm btn-action btn-register btn-wrap" onclick="return AspenDiscovery.Account.regInfoModal(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'library_market', '{$recordDriver->getExternalUrl()}');" style="width:70%"><i class="fas fa-external-link-alt" role="presentation"></i>{translate text="Registration Information" isPublicFacing=true}
								</a>
							{else}
								<a href="{$recordDriver->getExternalUrl()}" class="btn btn-sm btn-action btn-register btn-wrap" target="_blank" style="width:70%"><i class="fas fa-external-link-alt" role="presentation"></i> {translate text="Registration Information" isPublicFacing=true}</a>
							{/if}
				...
				{else}
					{if $recordDriver->isRegistrationRequired()}
						<div class="btn-group btn-group-vertical btn-block">
							{if !empty($recordDriver->getRegistrationModalBody())}
								<a class="btn btn-sm btn-action btn-register btn-wrap" onclick="return AspenDiscovery.Account.regInfoModal(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'library_market', '{$recordDriver->getExternalUrl()}');" style="width:70%">{translate text="Registration Information" isPublicFacing=true}
								</a>
							{else}
								<a href="{$recordDriver->getExternalUrl()}" class="btn btn-sm btn-action btn-register btn-wrap" target="_blank" style="width:70%"><i class="fas fa-external-link-alt" role="presentation"></i> {translate text="Registration Information" isPublicFacing=true}</a>
							{/if}
