			...
			<div id="vertical-menu-bar">
				...
				{if $loggedIn}{* Logged In *}
					<div class="menu-bar-option">
						<a href="#" onclick="AspenDiscovery.Menu.SideBar.showAccount(this)" class="menu-icon" title="Account">
							<div class="menu-bar-label rotated-text"><i class="fas fa-user fa-lg"></i><span class="rotated-text-inner">{translate text="Account"}</span></div>
						</a>
					</div>
				{else} {* Not Logged In *}
					<div class="menu-bar-option">
						<a href="/MyAccount/Home" id="loginLink" onclick="{if $isLoginPage}$('#username').focus();return false{else}return AspenDiscovery.Account.followLinkIfLoggedIn(this){/if}" data-login="true" class="menu-icon" title="{translate text='Login'}">
							<div class="menu-bar-label rotated-text"><i class="fas fa-sign-in-alt fa-lg"></i><span class="rotated-text-inner">{translate text="Login"}</span></div>
						</a>
					</div>
				{/if}
				<div class="menu-bar-option">
				...
				<script type="text/javascript">
					$(function(){ldelim}
						...
						{elseif (empty($isLoginPage) && !in_array($action, array('EmailResetPin', 'ResetPin', 'RequestPinReset', 'EmailPin', 'SelfReg', 'MyList', 'CiteList', 'PasswordRecovery'))) && ($module == "MyAccount" || $module == "Admin" || $module == "Circa" || $module == "Report" || ($module == 'Search' && $action == 'History'))}
