...
class EBSCOhost_Dashboard extends Admin_Dashboard
{
	function launch()
	{
		global $interface;

		$instanceName = $this->loadInstanceInformation('EbscohostRecordUsage');
		$this->loadDates();
		...
		$this->display('dashboard.tpl', 'EBSCOhost Dashboard');
	}
	...
	{
		$userUsage = new UserEbscohostUsage();
	...
	{
		$usage = new EbscohostRecordUsage();
		if (!empty($instanceName)){
		...
		$usage->selectAdd('COUNT(ebscohostId) as recordsUsed');
	...
	function getBreadcrumbs() : array
	{
		...
		$breadcrumbs[] = new Breadcrumb('/EBSCOhost/Dashboard', 'EBSCOhost Usage Dashboard');
		return $breadcrumbs;
	}
