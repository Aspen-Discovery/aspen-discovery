...
class SpellingWord extends DataObject
{
	...
	function getSpellingSuggestions($word){
		...
		//global $logger;
		//$logger->log("Loading spelling suggestions", Logger::LOG_DEBUG);
		...
		while ($this->fetch()){
			if ($this->word != $word){
				//$logger->log("Checking word {$this->word}", Logger::LOG_DEBUG);
				$levenshteinDistance = levenshtein($this->word, $word);
				//$logger->log("  Levenshtein Distance is $levenshteinDistance", Logger::LOG_DEBUG);
				similar_text($word, $this->word, $percent);
				//$logger->log("  Similarity is $percent", Logger::LOG_DEBUG);
				$stringPosition = strpos($this->word, $word);
				//$logger->log("  String Position is $stringPosition", Logger::LOG_DEBUG);
