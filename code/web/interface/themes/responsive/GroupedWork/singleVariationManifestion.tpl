<div class="col-sm-12">
    <div class="row">
        ...
        <div class="col-tn-9 col-xs-8{if !$viewingCombinedResults} col-md-5 col-lg-6{/if}">
            ...
            {if $relatedManifestation->getNumRelatedRecords() == 1}
                {include file='GroupedWork/copySummary.tpl' summary=$relatedManifestation->getItemSummary() totalCopies=$relatedManifestation->getCopies() itemSummaryId=$id recordViewUrl=$relatedManifestation->getUrl()}
            {else}
                {include file='GroupedWork/copySummary.tpl' summary=$relatedManifestation->getItemSummary() totalCopies=$relatedManifestation->getCopies() itemSummaryId=$id}
            {/if}
        </div>
    ...
    <div class="row">
        <div class="col-sm-12" id="relatedRecordPopup_{$id|escapeCSS}_{$relatedManifestation->format|escapeCSS}" style="display:none">
