			...
			{if $saved}
				...
				<table class="table table-bordered table-striped" width="100%">
					...
					<tr>
						<td>{$info.id}</td>
						<td>{$info.time}</td>
						<td>{$info.title}{if $info.hasNewResults}<a href="{$info.newTitlesUrl|escape}"><span class="badge badge-success">{translate text="New Titles!" isPublicFacing=true}</span></a>{/if}</td>
