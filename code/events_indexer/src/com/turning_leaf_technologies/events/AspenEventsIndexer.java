	...
	private void loadEvents() {
		try {
			...
			if (runFullUpdate) {
				...
				eventsStmt = aspenConn.prepareStatement("SELECT ei.*, e.title, e.description, e.eventTypeId, e.locationId, l.displayName, sl.name AS sublocationName, sl2.name AS sublocationOverride, e.sublocationId, COALESCE(NULLIF(e.cover, ''), et.cover) AS cover, e.private, e.hideTimestamps FROM event_instance AS ei LEFT JOIN event as e ON e.id = ei.eventID LEFT JOIN event_type AS et ON e.eventTypeId = et.id LEFT JOIN location AS l ON e.locationId = l.locationId LEFT JOIN sublocation AS sl on e.sublocationId = sl.id LEFT JOIN sublocation AS sl2 ON ei.sublocationId = sl2.id WHERE ei.date < ? AND ei.deleted = 0;");
			} else {
