...
<div class="controls table-responsive">
	<table id="{$propName}" class="{if $property.sortable}sortableProperty{/if} table table-striped table-sticky">
		<thead>
		<tr>
				...
				{foreach from=$property.structure item=subProperty}
					{if in_array($subProperty.type, array('text', 'enum', 'date', 'checkbox', 'integer', 'textarea', 'html')) }
						<td>
							...
							{else}
								<select name='{$propName}_{$subPropName}[{$subObject->id}]' id='{$propName}{$subPropName}_{$subObject->id}' class='form-control {if $subProperty.required == true} required{/if}' {if !empty($subProperty.onchange)}onchange="{$subProperty.onchange|replace:'~id~':$subObject->id}"{/if}>
