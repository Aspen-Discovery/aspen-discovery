...
- Add a new flag to determine if library settings are representing an entire consortium. When enabled, status will show as On Shelf for titles that are available, but not owned by the active library rather than Available at another location. (Ticket 87532)
