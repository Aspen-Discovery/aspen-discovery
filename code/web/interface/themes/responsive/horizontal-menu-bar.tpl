{strip}
	...
	<div class="menu-section menu-section-left">
		{if $useHomeLink == '1' || $useHomeLink == '3'}
			<a href="{$homeLink}" id="homeLink" class="menu-icon menu-bar-option" title="{translate text="Return to $homeLinkText" inAttribute=true}" aria-label="{translate text="Return to $homeLinkText" inAttribute=true}" role="link">
				<i class="fas fa-home fa-lg"></i><span class="menu-bar-label hidden-inline-block-xs"></span>
			</a>
		{/if}
		<a href="{if $useHomeLink == '0' || $useHomeLink == '2'}/{else}/Search/Home{/if}" id="homeLink" class="menu-icon menu-bar-option" title="{translate text='Browse the Catalog' inAttribute=true}" aria-label="{translate text='Browse the Catalog' inAttribute=true}" role="link">
			<i class="fas {if ($useHomeLink == '1' || $useHomeLink == '3') || ($showBookIcon == '1' && ($useHomeLink == '0' || $useHomeLink == '2'))}fa-book-open{else}fa-home{/if} fa-lg"></i>{if $useHomeLink == '1' || $useHomeLink == '3'}<span class="menu-bar-label visible-inline-block-lg">{$browseLinkText|translate}</span>{else}{/if}
		</a>
	...
	<div class="menu-section menu-section-right">
		{if $loggedIn}{* Logged In *}
			...
			<a tabindex="0" class="dropdown-toggle menu-icon menu-bar-option" role="button" title="{translate text="Account" inAttribute=true}" aria-haspopup="true" aria-expanded="false" id="account-menu-dropdown">
				{if $masqueradeMode}
		...
		<div class="dropdown menuToggleButton headerMenu" style="display:inline-block;"><a class="dropdown-toggle menu-icon menu-bar-option" tabindex="0" role="button" title="{translate text="Show Menu" inAttribute=true}"  aria-haspopup="true" aria-expanded="false" id="header-menu-dropdown">
