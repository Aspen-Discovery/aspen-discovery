...
class Admin_Libraries extends ObjectEditor {
	...
	public function getCopyOptionsFormStructure($activeObject) {
		...
		if ($activeObject instanceof Library) {
			...
			if ($activeObject->axis360ScopeId == -1 && empty($activeObject->getCloudLibraryScopes()) && $activeObject->hooplaScopeId == -1 && $activeObject->overDriveScopeId == -1 && $activeObject->palaceProjectScopeId == -1 && empty($activeObject->sideL)) {
