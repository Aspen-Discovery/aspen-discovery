...
require_once ROOT_DIR . '/sys/File/MARCBASE.php';
require_once ROOT_DIR . '/sys/File/MARC/Record.php';
require_once ROOT_DIR . '/sys/File/MARC/Field.php';
require_once ROOT_DIR . '/sys/File/MARC/Control_Field.php';
require_once ROOT_DIR . '/sys/File/MARC/Data_Field.php';
require_once ROOT_DIR . '/sys/File/MARC/Subfield.php';
require_once ROOT_DIR . '/sys/File/MARC/Exception.php';
	...
	private function _decode($text) {
		...
		if (preg_match("/^(\d{5})/", $text, $matches)) {
			...
			if ($record_length != strlen($text)) {
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INCORRECT_LENGTH], [
			...
			$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_NONNUMERIC_LENGTH], ["record_length" => substr($text, 0, 5)]));
		}
		...
		for ($n = 0; $n < $nfields; $n++) {
			...
			[
				,
				$tag,
			] = @unpack("A3", substr($dir, $n * File_MARC::DIRECTORY_ENTRY_LEN, File_MARC::DIRECTORY_ENTRY_LEN));
			[
				,
				$len,
			] = @unpack("A3/A4", substr($dir, $n * File_MARC::DIRECTORY_ENTRY_LEN, File_MARC::DIRECTORY_ENTRY_LEN));
			[
				,
				$offset,
			] = @unpack("A3/A4/A5", substr($dir, $n * File_MARC::DIRECTORY_ENTRY_LEN, File_MARC::DIRECTORY_ENTRY_LEN));

			// Check directory validity
			if (!preg_match("/^[0-9A-Za-z]{3}$/", $tag)) {
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_DIRECTORY_TAG], ["tag" => $tag]));
			}
			if (!preg_match("/^\d{4}$/", $len)) {
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_DIRECTORY_TAG_LENGTH], [
					"tag" => $tag,
					"len" => $len,
				]));
			}
			if (!preg_match("/^\d{5}$/", $offset)) {
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_DIRECTORY_OFFSET], [
					"tag" => $tag,
					"offset" => $offset,
				]));
			}
			if ($offset + $len > $record_length) {
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_DIRECTORY], ["tag" => $tag]));
			}
			...
			if (substr($tag_data, -1, 1) == File_MARC::END_OF_FIELD) {
				...
				$marc->addWarning(File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_FIELD_EOF], ["tag" => $tag]));
			}
			...
			if (preg_match("/^\d+$/", $tag) and ($tag < 10)) {
				...
				if (strlen($indicators) != 2) {
					$errorMessage = File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_INDICATORS], [
						"tag" => $tag,
				...
				foreach ($subfields as $subfield) {
					if (strlen($subfield) > 0) {
						...
						$errorMessage = File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_EMPTY_SUBFIELD], ["tag" => $tag]);
						$marc->addWarning($errorMessage);
					}
				}
