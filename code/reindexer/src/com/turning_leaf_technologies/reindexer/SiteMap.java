...
class SiteMap {
	...
	private int getVariableValue(String variableName, int defaultValue) throws SQLException {
		ResultSet rs = null;
		try {
			PreparedStatement st = dbConn.prepareStatement("SELECT value from variables WHERE name = ?");
			...
			PreparedStatement insertVariableStmt = dbConn.prepareStatement("INSERT INTO variables (`name`, `value`) VALUES ('" + variableName + "', ?)");
