		...
		<div class="col-sm-12" id="browse-category-feed" role="feed">
			...
			<div class="browse-category-feed-item" id="browse-category-{$browseCategory.textId}" role="article">
				<div class="browse-category-group-heading">
					{if !empty($browseCategory.searchUrl)}<a id="browse-search-link-{$browseCategory.textId}" href="{$browseCategory.searchUrl}" title="{translate text="View all results for %1%" 1=$browseCategory.label translateParameters=true isPublicFacing=true inAttribute=true}">{/if}<h2 id="tablist-browse-category-{$browseCategory.textId}"><i class="fas fa-search" role="presentation"></i> {translate text=$browseCategory.label isPublicFacing=true}</h2>{if !empty($browseCategory.searchUrl)}</a>{/if}
					{if !empty($isLoggedIn)}
