...
SELECT destination_grouped_work.full_title as destinationTitle, destination_grouped_work.author as destinationAuthor, destinationGroupedWorkId, GROUP_CONCAT(CONCAT(destination_grouped_work_primary_identifiers.type, ':', destination_grouped_work_primary_identifiers.identifier)) as destinationRecords, notes from merged_grouped_works INNER JOIN grouped_work as destination_grouped_work on destination_grouped_work.permanent_id = destinationGroupedWorkId inner join grouped_work_primary_identifiers as destination_grouped_work_primary_identifiers on destination_grouped_work_primary_identifiers.grouped_work_id = destination_grouped_work.id GROUP BY destinationTitle, destinationAuthor, destinationGroupedWorkId, notes INTO OUTFILE '/data/aspen-discovery/{sitename}/pika_export/mergedGroupedWorks.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '"';
