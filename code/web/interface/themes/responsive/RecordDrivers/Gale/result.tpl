{strip}
<div id="record{if !empty($summShortId)}{$summShortId}{else}{$summId|escape}{/if}" class="resultsList-gale resultsList row">
	{if !empty($showCovers)}
		<div class="coversColumn col-xs-3 col-sm-3{if empty($viewingCombinedResults)} col-md-3 col-lg-2{/if} text-center" aria-hidden="true" role="presentation">
			{if $disableCoverArt != 1 && !empty($bookCoverUrlMedium)}
				<a href="{$summUrl}" onclick="AspenDiscovery.Gale.trackGaleUsage('{$summId}')" target="_blank" aria-hidden="true" rel="noopener noreferrer">
	...
	<div class="{if empty($showCovers)}col-xs-12{else}col-tn-9 col-sm-9{if empty($viewingCombinedResults)} col-md-9 col-lg-10{/if}{/if}">
		<div class="result-title-row row">
			<div class="col-xs-12">
				...
				<a href="{$summUrl}" class="result-title notranslate" onclick="AspenDiscovery.Gale.trackGaleUsage('{$summId}')" target="_blank" rel="noopener noreferrer">
					{if !$recordDriver->getTitle()|removeTrailingPunctuation} {translate text='Title not available' isPublicFacing=true}{else}{$recordDriver->getTitle()|removeTrailingPunctuation|truncate:180:"..."|highlight}{/if}
				</a>
			</div>
		</div>
		...
		{if strlen($summSourceDatabase)}
			<div class="row result-source hidden-phone">
		...
		{if !empty($summHasFullText)}
			<div class="row result-full-text">
		...
		{if empty($viewingCombinedResults)}
			<div class="row">
				<div class="col-xs-12">
					{include file='Gale/result-tools-horizontal.tpl' recordUrl=$summUrl showMoreInfo=true}
				</div>
			</div>
		{/if}
	</div>
