...
export default class Settings_Notifications extends Component {
  ...
  getSavedPreferencesForDevice = () => {
    if (Constants.isDevice && this._isMounted) {
      ...
      if (notificationPreferences && deviceToken) {
        const devicePreferences = _.filter(notificationPreferences, [
          "token",
          deviceToken,
        ]);
  ...
  getSavedPreferences = async (token) => {
    ...
    const savedPreferences =
      route.params?.user.notification_preferences ?? null;
    if (savedPreferences) {
    ...
    for await (const pref of currentPreferences) {
      const savedValue = await getNotificationPreference(
        this.context.library.baseUrl,
        token,
        pref
      );
      if (savedValue) {
  ...
  updatePreference = async (option, newValue) => {
    ...
    if (token) {
      const updatedValue = await setNotificationPreference(
        this.context.library.baseUrl,
        token,
        option,
        newValue
  ...
  renderItem = (item) => {
    ...
    return (
      ...
      >
        ...
        <Switch
          onToggle={() => this.updatePreference(item.option, !item.allow)}
          isChecked={item.allow}
        />
      </HStack>
    );
  };
        ...
        <Box flex={1} safeArea={5}>
          ...
          >
            ...
            <Switch
              ...
              isChecked={this.state.allowNotifications}
              isDisabled={this.state.unableToNotify}
            />
          </HStack>
          {this.state.allowNotifications ? (
            <FlatList
              ...
              renderItem={({ item }) =>
                this.renderItem(this.state.categories[item])
              }
