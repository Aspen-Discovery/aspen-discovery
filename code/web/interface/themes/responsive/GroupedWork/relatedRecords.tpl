				...
				<td>
					<div class="btn-group btn-group-vertical btn-group-sm">
						...
						{foreach from=$relatedRecord->getActions() item=curAction}
							<a href="{if !empty($curAction.url)}{$curAction.url}{else}#{/if}" {if $curAction.onclick}onclick="{$curAction.onclick}"{/if} class="btn btn-sm btn-default btn-wrap" {if !empty($curAction.alt)}title="{$curAction.alt}"{/if}>{$curAction.title|translate}</a>
						{/foreach}
					</div>
				</td>
			</tr>
