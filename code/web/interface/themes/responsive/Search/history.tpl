			...
			{if $saved}
				...
				<table class="table table-bordered table-striped" width="100%">
					...
					<tr>
						<td>{$info.id}</td>
						<td>{$info.time}</td>
						<td>{$info.title}{if $info.hasNewResults}<a href="{$info.newTitlesUrl|escape}"><span class="label label-warning">{translate text="Updated" isPublicFacing=true}</span></a>{/if}</td>
