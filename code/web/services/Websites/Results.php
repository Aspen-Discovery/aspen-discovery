...
class Websites_Results extends ResultsAction {
	function launch() {
		...
		if ($searchObject->getResultTotal() < 1) {
			// No record found
			$interface->assign('subpage', 'Websites/list-none.tpl');
			...
			// Setup Display
			if ($displayMode == 'covers') {
				$displayTemplate = 'Websites/covers-list.tpl'; // structure for bookcover tiles
			} else {
				$displayTemplate = 'Websites/list-list.tpl'; // structure for regular results
		...
		$interface->assign('exploreMoreSection', 'websites');
