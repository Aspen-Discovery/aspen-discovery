...
class SSOAuthentication implements Authentication {
	...
	public function authenticate($validatedViaSSO) {
		if(!$validatedViaSSO) {
			...
			return $this->getValidatedAspenUser($_REQUEST['username']);
		}
	}
		...
		if ((($username == '') || ($password == '')) && $this->ssoSettings->service === 'ldap') {
			...
			if($this->ssoSettings->service) {
				if ($this->ssoSettings->service === 'ldap') {
					...
					return $ldapAuthentication->validateAccount($username, $password);
				}
	...
	private function getValidatedAspenUser($username) {
