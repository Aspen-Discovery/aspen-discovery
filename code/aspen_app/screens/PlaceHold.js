...
export default class PlaceHold extends Component {
  ...
  onPressItem = async(selectedItem) => {
    ...
    const url = this.state.pathUrl + '/app/aspenPlaceHold.php?library=' + this.state.pathLibrary + '&barcode=' + this.state.username + '&pin=' + this.state.password + '&item=' + selectedItem + '&location=' + this.state.pickUpLocation + '&rand=' + random;
    ...
    fetch(url)
      ...
      .then(res => {
        ...
        if (holdStatus != '1') {
          this.setState({ holdMessage: holdMessage, showFailure: true });
          return '';
        }
        ...
        <View style={ Stylesheet.outerContainer }>
          ...
          <View style={ Stylesheet.ilsMessageContainer }>
            <TouchableOpacity onPress={ () => this.props.navigation.goBack() }>
              {this.state.showSuccess ? <Text style={ Stylesheet.ilsSuccessMessage }>{ this.state.holdMessage }{'\n\n'}Tap to close.</Text> : null}
              {this.state.showFailure ? <Text style={ Stylesheet.ilsFailMessage }>{ this.state.holdMessage }{'\n\n'}Tap to close.</Text> : null}
            </TouchableOpacity>
          </View> : null }
