...
export default class MyAccount extends Component {
     ...
     _fetchILSMessages = async () => {
          ...
          await getILSMessages().then((response) => {
               if (response === 'TIMEOUT_ERROR') {
                    ...
                    const messageCount = response.length;
                    this.setState({
                         hasError: false,
                         error: null,
                         isLoading: false,
     ...
     _renderSettingsHeader = () => {
          return (
               <Box safeArea={5}>
                    <Center>
                         <Text fontSize="xl">
                              {this.state.thisPatron} {translate('user_profile.title')}
                         </Text>
                         {this.state.barcode ? (
                              <HStack space={1} alignItems="center" mt={2} mb={2}>
                                   <Icon as={Ionicons} name="card" size="sm" />
                    ...
                    <HStack space={1} pb={10}>
                         <VStack width="50%">
                              <Center>
                                   <Text fontSize="md" mb={1}>
                                        <Text bold>{translate('checkouts.title')}: </Text>
                                        {this.state.numCheckedOut}
                                   </Text>
                                   {this.state.numOverdue > 0 ? (
                                        <Badge colorScheme="danger" rounded="4px">
                                             <Text fontSize="xs" bold>
                                                  {translate('checkouts.overdue_summary', {
                                                       count: this.state.numOverdue,
                                                  })}
                                             </Text>
                                        </Badge>
                                   ) : null}
                         ...
                         <VStack width="50%">
                              <Center>
                                   <Text fontSize="md" mb={1}>
                                        <Text bold>{translate('holds.holds')}: </Text>
                                        {this.state.numHolds}
                                   </Text>
                                   {this.state.numHoldsAvailable > 0 ? (
                                        <Badge colorScheme="success" rounded="4px">
                                             <Text fontSize="xs" bold>
                                                  {translate('holds.ready_for_pickup', {
                                                       count: this.state.numHoldsAvailable,
                                                  })}
                                             </Text>
                                        </Badge>
                                   ) : null}
     ...
     render() {
          ...
          return (
               <Box pb={10}>
                    <FlatList data={this.state.defaultMenuItems} renderItem={({ item }) => this.renderNativeItem(item)} keyExtractor={(item, index) => index.toString()} ListHeaderComponent={this._renderSettingsHeader()} ListFooterComponent={this._renderSettingsFooter()} refreshControl={<RefreshControl refreshing={this.state.isRefreshing} onRefresh={this._onRefresh.bind(this)} />} />
               </Box>
          );
     }
}