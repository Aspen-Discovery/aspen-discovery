	...
	<div class="col-tn-12 col-xs-12 col-sm-8 col-md-9 col-lg-9">
		...
		<div class="col-tn-4" style="display:flex; justify-content:center;">
					...
					<div class="btn-group btn-group-vertical btn-block">
						...
						{else}
							{if !empty($recordDriver->getRegistrationModalBody())}
								<a class="btn btn-sm btn-action btn-wrap" onclick="return AspenDiscovery.Account.regInfoModal(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'communico', '{$recordDriver->getExternalUrl()}');" style="width:70%">{translate text="Registration Information" isPublicFacing=true}
								</a>
							{else}
			...
			{else}
				...
				{else}
					<a class="btn btn-sm btn-action btn-wrap" style="width:70%" onclick="return AspenDiscovery.Account.saveEvent(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'communico');">{translate text="Add to Your Events" isPublicFacing=true}</a>
				{/if}
			{/if}
		</div>
		</div>
