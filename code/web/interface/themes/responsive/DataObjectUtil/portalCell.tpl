	...
	<div class="row portal-cell-title-edit">
		...
		{if empty($objectAction) || $objectAction != 'copy'}
			<div class="col-xs-4 text-right">
				{if $portalCell->weight != 0}<span class="btn btn-xs btn-default" onclick="return AspenDiscovery.WebBuilder.moveCell('{$portalCell->id}', 'left');" title="{translate text="Move Left" isAdminFacing=true inAttribute=true}"><i class="fas fa-caret-left fa {if $isRTL}fa-flip-horizontal{/if}"></i></span>{/if}
				...
				{if !$portalCell->isLastCell()}<span class="btn btn-xs btn-default" onclick="return AspenDiscovery.WebBuilder.moveCell('{$portalCell->id}', 'right');" title="{translate text="Move Right" isAdminFacing=true inAttribute=true}"><i class="fas fa-caret-right fa {if $isRTL}fa-flip-horizontal{/if}"></i></span>{/if}
			</div>
		{/if}
	</div>
	<div class="row">
