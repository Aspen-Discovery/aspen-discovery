				...
				<div class="archiveComponentBox">
					...
					<div class="archiveComponentLinks row">
						{foreach from=$browseCollectionTitlesData.collectionTitles item=titleInfo key=indexNumber}
							<div class="col-tn-6"><a href="{$titleInfo.link}"{if $isCollectionOnExhibitPage && $browseCollectionTitlesData.collectionPid} onclick="AspenDiscovery.Archive.setForExhibitNavigation({$indexNumber}, 1, '{$browseCollectionTitlesData.collectionPid}')"{/if}>{$titleInfo.title}</a></div>
						{/foreach}
					</div>
				</div>
			</div>
