									...
									<tr>
										...
										{if $finesToPay == 2 && $fineTotalsVal.$userId > $minimumFineAmount && $fine.canPayFine !== false}
											...
											{else}
												...
												{if !empty($overPayWarning)}<span id="overPayWarning" class="text-danger" style="display:none;"><i class="fas fa-exclamation-triangle"></i> {$overPayWarning}</span>{/if}</td>
											{/if}
