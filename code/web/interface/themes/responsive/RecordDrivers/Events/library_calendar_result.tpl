							...
							<div class="btn-toolbar">
								<div class="btn-group btn-group-vertical btn-block">
									{if !empty($recordDriver->getRegistrationModalBody())}
										<a class="btn btn-sm btn-action btn-wrap" onclick="return AspenDiscovery.Account.regInfoModal(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'library_market', '{$recordDriver->getExternalUrl()}');"><i class="fas fa-external-link-alt"></i>{translate text="Registration Information" isPublicFacing=true}
										</a>
									{else}
									...
									{/if}
									<a onclick="return AspenDiscovery.Account.saveEvent(this, 'Events', '{$recordDriver->getUniqueID()|escape}', 'library_market');" class="btn btn-sm btn-action btn-wrap" style="width:100%">{translate text="Add to Your Events" isPublicFacing=true}</a>
								</div>
							</div>
							<br>
						{else}
