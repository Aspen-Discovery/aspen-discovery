...
class OverDriveProcessor implements AutoCloseable {
					...
					if (productRS.getInt("deleted") == 1) {
						...
						try (ResultSet numCopiesRS = getNumCopiesStmt.executeQuery()) {
							...
							if (numCopiesRS.getInt("totalOwned") == 0) {
								...
								HashMap<String, OverDriveAvailabilityInfo> availabilityInfo;
								HashSet<Long> uniqueSettings;
								try (ResultSet availabilityRS = getProductAvailabilityStmt.executeQuery()) {
									availabilityInfo = new HashMap<>();
									uniqueSettings = new HashSet<>();
	...
	private String fixOverDriveMetaData(long productId) throws SQLException {
		...
		try (ResultSet doubleDecodeRawResponseRS = doubleDecodeRawMetadataStmt.executeQuery()) {
	...
	private String loadOverDriveSubjects(AbstractGroupedWorkSolr groupedWork, JSONObject productMetadata) throws JSONException {
		...
		//noinspection DuplicatedCode This is Duplicated in Axis360Processor
	...
	public void close() {
		try {
			getProductInfoStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
		try {
			getNumCopiesStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
		try {
		...
		try {
			getProductAvailabilityStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
		try {
			getProductFormatsStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
		try {
			doubleDecodeRawMetadataStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
		try {
			updateRawMetadataStmt.close();
		} catch (SQLException e) {
			logger.error(e);
		}
	}
