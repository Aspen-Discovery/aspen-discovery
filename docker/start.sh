...
if [ ! -z "$DATABASE_ROOT_PASSWORD" ] ; then
	mysql -u"$DATABASE_ROOT_USER" -p"$DATABASE_ROOT_USER" -h"$DATABASE_HOST" -P"$DATABASE_PORT" -e "CREATE USER '${DATABASE_USER}'@'%' IDENTIFIED BY '${DATABASE_PASSWORD}'; GRANT ALL PRIVILEGES ON ${DATABASE_NAME}.* TO '${DATABASE_USER}'@'%'; FLUSH PRIVILEGES;"
fi
