...
abstract class IslandoraRecordDriver extends IndexRecordDriver {
	protected $pid = null;
...
//		$interface->assign('summUrl', $this->getLinkUrl());
	...
	public function getBaseMoreDetailsOptions($isbn_ignored = null){
		...
		if (strlen($description)) {
			...
			if ($this instanceof PersonRecordDriver) {
		...
		if ($this->relatedCollections == null){
			...
			if ($this->isEntity()){
				...
				foreach ($directlyLinkedObjects['objects'] as $tmpObject){
				    /** @var IslandoraRecordDriver $driver */
				    $driver = $tmpObject['driver'];
					$linkedCollections = $driver->getRelatedCollections();
			...
			if (count($this->relatedCollections) == 0){
				foreach ($collectionsRaw as $collectionInfo) {
					if (!$fedoraUtils->isPidValidForPika($collectionInfo['object']['value'])){
						...
						if ($parentDriver && $parentDriver instanceof IslandoraRecordDriver){
				...
				foreach ($response['response']['docs'] as $doc) {
					...
					if (isset($doc['mods_extension_marmotLocal_relatedPersonOrg_entityPid_ms']) && isset($doc['mods_extension_marmotLocal_relatedPersonOrg_role_ms'])){
						...
						if (count($doc['mods_extension_marmotLocal_relatedPersonOrg_entityPid_ms']) != count($doc['mods_extension_marmotLocal_relatedPersonOrg_role_ms'])){
							/** @var IslandoraRecordDriver $relatedEntityDriver */
					...
					if ($entityDriver instanceof EventRecordDriver) {
					...
					}elseif ($entityDriver instanceof PersonRecordDriver){
						//Reverse roles as appropriate
						if ($role == 'child'){
							$role = 'parent';
						}elseif ($role == 'parent'){
					...
					}elseif ($entityDriver instanceof OrganizationRecordDriver){
						$this->addRelatedEntityToArrays($entityDriver->getUniqueID(), $entityDriver->getTitle(), 'organization', '', $role);
					}elseif ($entityDriver instanceof PlaceRecordDriver){
	...
	private function addRelatedEntityToArrays($pid, $entityName, $entityType, $note, $role, $isCreator = false) {
		...
		if (strlen($pid) == 0 || strpos($pid, ':') === false){
			...
			if (!in_array($entityType, array('person', 'organization', 'event', 'place'))){
				...
				if ($recordDriver instanceof PersonRecordDriver) {
					$entityType = 'person';
				}elseif ($recordDriver instanceof PlaceRecordDriver){
					$entityType = 'place';
				}elseif ($recordDriver instanceof EventRecordDriver){
					$entityType = 'event';
				}elseif ($recordDriver instanceof OrganizationRecordDriver){
			...
			if ($entityType == 'person'){
				...
				if (strlen($role) > 0 && !in_array(strtolower($role), IslandoraRecordDriver::$nonProductionTeamRoles)) {
					$this->addEntityToArray($pid, $entityInfo, $this->productionTeam);
				}elseif (strlen($role) > 0 && in_array(strtolower($role), IslandoraRecordDriver::$brandingRoles)){
					if ($role == 'owner'){
				...
				if (strlen($role) > 0 && in_array(strtolower($role), IslandoraRecordDriver::$brandingRoles)){
					if ($role == 'owner'){
						$entityInfo['sortIndex'] = 1;
						$entityInfo['label'] = 'Owned by ' . $entityInfo['label'];
					}elseif ($role == 'donor'){
                    ...
                    if ($degreeGrantorRaw) {
                        ...
                        if ($personPid) {
                            ...
                            if ($educationPersonObject) {
                                /** @var IslandoraRecordDriver $educationPersonDriver */
