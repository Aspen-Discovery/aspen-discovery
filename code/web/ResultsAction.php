...
abstract class ResultsAction extends Action
{
	function getResultsBreadcrumbs($searchType)
	{
		...
		if (empty($recordCount)) {
			$resultCountText = translate("No Results Found");
		}else{
			if ($interface->getVariable('displayMode') == 'covers') {
				$resultCountText = translate(['text' => "There are %1% total results.", 1=>number_format($recordCount)]);
			}else{
				...
				$resultCountText = translate(['text'=>"Showing %1% - %2% of %3%", 1=>$recordStart, 2=>$recordEnd, 3=>$recordCount]);
			}
		}
