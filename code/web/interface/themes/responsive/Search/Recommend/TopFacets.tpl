							...
							<div class="categoryValue categoryValue_{translate inAttribute=true text=$thisFacet.value|lower|replace:' ':'' isPublicFacing=true} col-tn-2">
								<a href="{$thisFacet.url|escape}">
									<div class="row">
										...
										<div class="col-xs-6 formatCategoryLabel">
											{translate text=$thisFacet.display|escape isPublicFacing=true}{if $facetCountsToShow == 1 || ($facetCountsToShow == 2 && !$thisFacet.countIsApproximate)}<br>({if !empty($thisFacet.countIsApproximate)}{/if}{$thisFacet.count|number_format:0:".":","}){/if}
										</div>
									</div>
								</a>
							</div>
