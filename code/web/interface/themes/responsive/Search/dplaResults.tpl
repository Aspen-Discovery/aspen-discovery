{strip}
	<div id="dplaSearchResults">
		...
		<h2>{translate text="More from Digital Public Library of America"}</h2>
					...
					<div class="col-xs-2">
						{if $result.object}
							<a href="{$result.link}" aria-hidden="true">
								<img src="{$result.object}" class="listResultImage img-thumbnail img-responsive" alt="{$result.title}{if !empty($result.publisher)} {translate text="from"} {$result.publisher}{/if}"/>
							</a>
						{/if}
					</div>
				{/if}
