				...
				<div class="striped">
					{foreach from=$prospectorResults item=prospectorResult}
						<div class="result">
							...
							<div class="resultItemLine2">{if !empty($prospectorResult.author)}by {$prospectorResult.author} {/if}{if !empty($prospectorResult.pubDate)}Published {$prospectorResult.pubDate}{/if}</div>
						</div>
					{/foreach}
				</div>
