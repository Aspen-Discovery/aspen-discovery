<div id="page-content" class="content">
	<div id="main-content">
		<h1>{translate text='Materials Request'}</h1>
		<div id="materialsRequest">
			<div class="materialsRequestExplanation alert alert-info">
				{if empty($newMaterialsRequestSummary)}
					{translate text='If you cannot find a title in our catalog, you can request the title via this form.
				...
				{else}
					{translate text=$newMaterialsRequestSummary}
				{/if}
			</div>
			<form id="materialsRequestForm" action="/MaterialsRequest/Submit" method="post" class="form form-horizontal" role="form">
				...
				<div class="materialsRequestLoggedInFields" {if !$loggedIn}style="display:none"{/if}>
					...
					<div id="copyrightAgreement" class="formatSpecificField articleField col-sm-9 col-sm-offset-3">
						<label for="acceptCopyrightYes"><input type="radio" name="acceptCopyright" class="required" id="acceptCopyrightYes" value="1">{translate text="Accept"}</label>
						<label for="acceptCopyrightNo"><input type="radio" name="acceptCopyright" id="acceptCopyrightNo" value="1">{translate text="Decline"}</label>
					</div>
					<div class="col-sm-9 col-sm-offset-3">
						<input type="submit" value="{translate text='Submit Materials Request' inAttribute=true}" class="btn btn-primary">
					</div>
				</div>
			</form>
		</div>
