...
class GroupedWorksSolrConnector2 extends Solr
{
	...
	function getRecordByBarcode($barcode)
	{
		...
		$options = array('q' => "barcode:\"$barcode\"", 'fl' => SearchObject_GroupedWorkSearcher2::$fields_to_return);
	...
	function searchForRecordIds($ids)
	{
		...
		$options = array('q' => $idString, 'rows' => count($ids), 'fl' => SearchObject_GroupedWorkSearcher2::$fields_to_return);
		$result = $this->_select('GET', $options);
		if ($result instanceof AspenError) {
			AspenError::raiseError($result);
		}
