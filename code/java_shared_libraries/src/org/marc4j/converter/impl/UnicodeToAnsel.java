...
public class UnicodeToAnsel extends CharConverter {
    ...
    boolean doNotChangeCharset = false;

    /**
     * Creates a new instance and loads the MARC4J supplied Ansel/Unicode conversion tables based on the official LC
     * tables. Loads in the generated class ReverseCodeTableGenerated which contains switch statements to look up the
     ...
     * <a href="http://www.loc.gov/marc/specifications/codetables.xml">...</a>).
     *
     * @param pathname - the name of a file to read to build the ReverseCodeTable instead of the standard LC table
     */
     ...
     * <a href="http://www.loc.gov/marc/specifications/codetables.xml">...</a>).
     *
     * @param in - an InputStream to read to build the ReverseCodeTable instead of the standard LC table
     */
    ...
    private void convertPortion(final char[] data, final StringBuffer sb) {
	    ...
	    for (final Character c : data) {
		    ...
		    if (charValue == 0x20 && rct.getPreviousG0() != '1') {
                ...
                String substring = Integer.toHexString(charValue + 0x10000).toUpperCase().substring(1);
                if (!rct.charHasMatch(c)) {
                    // Unicode character c has no match in the Marc8 tables. Try unicode-decompose on it
                    // to see whether the decomposed form can be represented. If when decomposed, all
                    ...
                    // re-compose the main character with the first diacritic, and check whether that
                    ...
                    if (charValue < 0x1000) {
                        marc.append("&#x").append(substring).append(";");
                    } else {
                        marc.append("&#x").append(Integer.toHexString(charValue).toUpperCase()).append(";");
                        // continue;
                    }
                    ...
                    // character sets for Basic and extended latin are preloaded, usually if a character
