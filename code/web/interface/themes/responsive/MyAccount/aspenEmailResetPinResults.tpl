				...
				{elseif $result.message}
					...
					<form method="post" role="form" action="/MyAccount/CompletePinReset">
						<div class="form-group">
							<label for='token'>{translate text="Reset Token" isAdminFacing=true}</label>
							<input type='text' name='token' id='token' value='' maxlength=12 class='form-control required'>
						</div>
						<div class="form-group">
							<button type="submit" name="submit" value="Next" class="btn btn-primary">{translate text="Next" isPublicFacing=true}</button>
						</div>
					</form>
				{else}
