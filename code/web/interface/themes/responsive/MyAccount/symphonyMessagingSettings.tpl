	...
	{else}
		<form method="post" action="/MyAccount/MessagingSettings" name="kohaMessaging">
			...
			<table class="table table-bordered table-condensed table-striped" id="messagingTable">
				<thead>
					<tr>
						<th>{translate text="Phone Label (ex. my cell)"}</th>
						<th>{translate text="Phone Number"}</th>
						<th>{translate text="Country Code"}</th>
						<th>{translate text="Alerts to Receive"}</th>
						<th></th>
					</tr>
				</thead>
				<tbody>
				...
				{foreach from=$phoneList key=phoneIndex item=phone}
					<tr id="phoneRow{$phoneIndex}" {if $phoneIndex != 1 && $phone.enabled == false}style="display: none"{/if}>
						...
						<td>
							<div><label for="billNotices_{$phoneIndex}"><input type="checkbox" name="billNotices[{$phoneIndex}]" id="billNotices_{$phoneIndex}" {if $phone.billNotices}checked{/if}>{translate text="Bill Notices"}</label></div>
							<div><label for="overdueNotices_{$phoneIndex}"><input type="checkbox" name="overdueNotices[{$phoneIndex}]" id="overdueNotices_{$phoneIndex}" {if $phone.overdueNotices}checked{/if}>{translate text="Overdue Notices"}</label></div>
							<div><label for="holdPickupNotices_{$phoneIndex}"><input type="checkbox" name="holdPickupNotices[{$phoneIndex}]" id="overdueNotices_{$phoneIndex}" {if $phone.holdPickupNotices}checked{/if}>{translate text="Hold Pickup Notices"}</label></div>
							<div><label for="manualMessages_{$phoneIndex}"><input type="checkbox" name="manualMessages[{$phoneIndex}]" id="manualMessages_{$phoneIndex}" {if $phone.manualMessages}checked{/if}>{translate text="Manual Messages"}</label></div>
							<div><label for="generalAnnouncements_{$phoneIndex}"><input type="checkbox" name="generalAnnouncements[{$phoneIndex}]" id="generalAnnouncements_{$phoneIndex}" {if $phone.generalAnnouncements}checked{/if}>{translate text="General Announcements"}</label></div>
						</td>
						<td><button class="btn btn-sm btn-danger" onclick="$('#phoneRow{$phoneIndex}').hide();$('#phoneNumberDeleted{$phoneIndex}').val('true');return false;">{translate text="Delete"}</button> </td>
					</tr>
				{/foreach}
			...
			{if $canSave}
				<button class="btn btn-sm btn-primary" name="addPhone" id="addPhoneBtn" onclick="return addPhoneRow();">{translate text="Add Phone Number"}</button>
