...
class File_MARC_Record {
	...
	private function _buildDirectory() {
		 ...
		 */
		$base_address = File_MARC::LEADER_LEN +    // better be 24
			(count($directory) * File_MARC::DIRECTORY_ENTRY_LEN) + // all the directory entries
			1;              // end-of-field marker


		$total = $base_address +  // stuff before first field
	...
	function getField($spec = null, $pcre = null) {
		foreach ($this->fields as $field) {
			if (($pcre && preg_match("/$spec/", $field->getTag())) || (!$pcre && $spec == $field->getTag())) {
				return $field;
			}
		}
		return false;
	...
	function deleteFields($tag, $pcre = null) {
		...
		foreach ($this->getFields() as $field) {
			if (($pcre && preg_match("/$tag/", $field->getTag())) || (!$pcre && $tag == $field->getTag())) {
				$field->delete();
				$cnt++;
			}
		}
	...
	public function jsonDecode($rawData): bool {
			...
			if (!$field->isEmpty()) {
				switch (get_class($field)) {
					...
					case "File_MARC_Data_Field":
						...
						foreach ($field->getSubfields() as $sf) {
							$subs[] = [
								utf8_encode($sf->getCode()),
								utf8_encode($sf->getData()),
							];
						}
						$contents = [
							utf8_encode($field->getTag()),
