...
export default class Facet extends Component {
  ...
  componentDidMount = async () => {
    ...
    const data = _.filter(SEARCH.availableFacets.data, [
      "field",
      this.state.category,
    ]);
    if (data[0]) {
  ...
  render() {
    ...
    if (
      ...
      category === "lexile_score" ||
      category === "accelerated_reader_point_value" ||
      category === "accelerated_reader_reading_level"
    ) {
      return (
        <View style={{ flex: 1 }}>
          <ScrollView>
