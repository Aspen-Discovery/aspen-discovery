...
import { Actionsheet, Badge, Box, Button, Center, Checkbox, CheckIcon, FlatList, FormControl, HStack, Icon, IconButton, Image, Modal, Pressable, Select, Text, useDisclose, VStack } from 'native-base';
...
import { getHolds, getProfile, PATRON, reloadHolds } from '../../util/loadPatron';
...
export default class Holds extends Component {
     ...
     _fetchHolds = async () => {
          ...
          this._isMounted &&
               (await getHolds(this.context.library.baseUrl).then((r) => {
                    this.setState({
                         holds: r['holds'],
     ...
     _reloadHolds = async () => {
          ...
          this._isMounted &&
               (await reloadHolds(this.context.library.baseUrl).then((r) => {
                    this.setState({
                         holds: r['holds'],
                         holdsNotReady: r['holdsNotReady'],
                         holdsReady: r['holdsReady'],
     ...
     openGroupedWork = (item, libraryUrl) => {
          this.props.navigation.navigate('GroupedWork', { item: item, url: libraryUrl });
     };
     ...
     renderHoldItem = (item, libraryUrl, user) => {
          return (
               <HoldItem
                    data={item}
     ...
     _listFooterComponent = (libraryUrl) => {
          ...
          if (showSelectOptions) {
               return (
                    <Center mt={5} mb={5}>
                         <ManageSelectedHolds selectedValues={this.state.groupValues} libraryUrl={libraryUrl} updateProfile={this.updateProfile} _fetchHolds={this._fetchHolds} onAllDateChange={this.onDateChange} selectedReactivationDate={this.state.selectedStartDate} clearGroupValue={this.clearGroupValue} />
                         <IconButton
                              _icon={{ as: MaterialIcons, name: 'refresh', color: 'coolGray.500' }}
                              onPress={() => {
                                   this._reloadHolds();
                              }}
                         />
          ...
          return (
               <Center mt={5} mb={2}>
                    <ManageAllHolds data={this.state.holds} libraryUrl={libraryUrl} updateProfile={this.updateProfile} _fetchHolds={this._fetchHolds} onDateChange={this.onDateChange} selectedReactivationDate={this.state.selectedStartDate} clearGroupValue={this.clearGroupValue} />
                    <IconButton
                         _icon={{ as: MaterialIcons, name: 'refresh', color: 'coolGray.500' }}
                         onPress={() => {
                              this._reloadHolds();
                    ...
                    <Box>
                         <Center pt={5} pb={10}>
                              <Checkbox.Group
                                   ...
                                   onChange={(values) => {
                                        this.setGroupValue(values);
                                   }}>
                                   <FlatList data={holds} ListEmptyComponent={this._listEmptyComponent()} ListFooterComponent={this._listFooterComponent(library.baseUrl)} renderItem={({ item }) => this.renderHoldItem(item, library.baseUrl, user)} keyExtractor={(item) => item.id.concat('_', item.position)} />
                              </Checkbox.Group>
                         </Center>
                    </Box>
               </SafeAreaView>
...
function HoldItem(props) {
     let expirationDate;
     let availableDate;
     const { data, locations, openGroupedWork, translations, libraryUrl, updateProfile, discoveryVersion, userProfile, onDateChange, selectedReactivationDate, _fetchHolds, clearGroupValue } = props;
          ...
          <>
               <Pressable onPress={onOpen} borderBottomWidth="1" _dark={{ borderColor: 'gray.600' }} borderColor="coolGray.200" pl="4" pr="5" py="2">
                    <HStack space={3}>
                         {data.coverUrl && data.source !== 'vdx' ? (
                              <VStack>
                                   ...
                                   {data.allowFreezeHolds && cancelable && allowLinkedAccountAction ? (
                                        <Center>
                                             <Checkbox value={method + '|' + data.recordId + '|' + data.cancelId + '|' + data.source + '|' + data.userId} my={3} size="md" accessibilityLabel="Check item"></Checkbox>
                                        </Center>
                                   ) : null}
                              </VStack>
                         ) : null}
                         ...
                         <VStack maxW="80%">
                              <Text bold mb={1} fontSize={{ base: 'sm', lg: 'lg' }}>
                                   {title}
                              </Text>
                              {data.frozen ? (
                                   <Text>
                              ...
                              {data.source === 'ils' ? (
                                   <Text fontSize={{ base: 'xs', lg: 'sm' }}>
                                        <Text bold>{translations.pickUpLocation}:</Text> {data.currentPickupName}
                                   </Text>
                              ) : null}
                              {data.available ? (
                                   <Text fontSize={{ base: 'xs', lg: 'sm' }}>
                                        <Text bold>{translations.pickupBy}:</Text> {expirationDate}
                                   </Text>
                              ) : null}
                              {!data.available && data.position ? (
                                   <Text fontSize={{ base: 'xs', lg: 'sm' }}>
                                        <Text bold>{translations.position}:</Text> {data.position}
                                   </Text>
                              ) : null}
                              {data.status && data.source === 'vdx' ? (
                                   <Text fontSize={{ base: 'xs', lg: 'sm' }}>
                                        <Text bold>{translate('holds.status')}:</Text> {data.status}
                                   </Text>
                              ) : null}
                         </VStack>
                    </HStack>
               ...
               <Actionsheet isOpen={isOpen} onClose={onClose} size="full">
                    <Actionsheet.Content>
                         ...
                         {data.allowFreezeHolds === '1' && allowLinkedAccountAction && data.frozen === false ? <SelectThawDate handleOnDateChange={onDateChange} onClose={onClose} freezeId={data.cancelId} recordId={data.recordId} source={data.source} libraryUrl={libraryUrl} userId={data.userId} reactivationDate={selectedReactivationDate} _fetchHolds={_fetchHolds} updateProfile={updateProfile} clearGroupValue={clearGroupValue} /> : ''}
                         ...
                         {updateLocation && allowLinkedAccountAction ? <SelectPickupLocation locations={locations} onClose={onClose} _fetchHolds={_fetchHolds} userId={data.userId} currentPickupId={data.pickupLocationId} holdId={data.cancelId} label={translations.changePickUpLocation} libraryUrl={libraryUrl} /> : ''}
                    </Actionsheet.Content>
               </Actionsheet>
          </>
     );
...
const SelectThawDate = (props) => {
     const { onClose, freezeId, recordId, source, libraryUrl, userId, _fetchHolds, data, count, updateProfile, clearGroupValue } = props;
                    ...
                    <Actionsheet.Content>
                         {numToCancel > 0 ? (
                              <Actionsheet.Item
                                   ...
                                   <Text>Cancel holds ({numToCancel})</Text>
                              </Actionsheet.Item>
                         ) : (
                         ...
                         {numToFreeze > 0 ? (
                              <SelectThawDate count={numToFreeze} data={titlesToFreeze} handleOnDateChange={onAllDateChange} libraryUrl={libraryUrl} reactivationDate={selectedReactivationDate} _fetchHolds={_fetchHolds} onClose={onClose} updateProfile={updateProfile} clearGroupValue={clearGroupValue} />
                         ) : (
                              <Actionsheet.Item isDisabled>
                                   <Text>Freeze holds ({numToFreeze})</Text>
                              </Actionsheet.Item>
                         )}
                         {numToThaw > 0 ? (
                              <Actionsheet.Item
                                   ...
                                   <Text>Thaw holds ({numToThaw})</Text>
                              </Actionsheet.Item>
                         ) : (
                              <Actionsheet.Item isDisabled>
                                   <Text>Thaw holds ({numToThaw})</Text>
...
const ManageAllHolds = (props) => {
     const { data, libraryUrl, onDateChange, selectedReactivationDate, updateProfile, _fetchHolds, clearGroupValue } = props;
     const { isOpen, onOpen, onClose } = useDisclose();
     const [loading, setLoading] = useState(false);
     const [cancelling, startCancelling] = useState(false);
     const [thawing, startThawing] = useState(false);
               ...
               <Actionsheet isOpen={isOpen} onClose={onClose}>
                    <Actionsheet.Content>
                         <Actionsheet.Item
                              ...
                              <Text>Cancel all holds ({numToCancel})</Text>
                         </Actionsheet.Item>
                         <Actionsheet.Item>
                              <SelectThawDate count={numToFreeze} data={titlesToFreeze} handleOnDateChange={onDateChange} libraryUrl={libraryUrl} reactivationDate={selectedReactivationDate} updateProfile={updateProfile} _fetchHolds={_fetchHolds} clearGroupValue={clearGroupValue} />
                         </Actionsheet.Item>
                         <Actionsheet.Item
                              isLoading={thawing}
                              isLoadingText="Thawing..."
                              ...
                              <Text>Thaw all holds ({numToThaw})</Text>
                         </Actionsheet.Item>
                    </Actionsheet.Content>
               </Actionsheet>
          </Center>
