	...
	function doBiblioMapping() : array {
		...
		if (isset($_FILES['mappingFile'])) {
			...
			if (isset($mappingFile["error"]) && $mappingFile["error"] == 4) {
				$result['message'] = translate(['text'=>"No Mapping file was uploaded", 'isAdminFacing'=>true]);
			} else if (isset($mappingFile["error"]) && $mappingFile["error"] > 0) {
				$result['message'] =  translate(['text'=>"Error in file upload for mapping file %1%", 1=>$mappingFile["error"], "isAdminFacing"=>true]);
			} else {
