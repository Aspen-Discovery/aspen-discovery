									...
									<form action="" method="post" class="form-horizontal" id="contactUpdateForm">
										...
										{if $showUsernameField}
											<div class="form-group">
												...
												<div class="col-xs-8">
													<input type="text" name="username" id="username" value="{if !is_numeric(trim($profile->alt_username))}{$profile->alt_username|escape}{/if}" size="25" maxlength="25" class="form-control">
										...
										{if $ils == 'CarlX'} {* CarlX Notification Options *}
											<div class="form-group">
												...
												<div class="col-xs-8">
													{if $edit == true && $canUpdateContactInfo == true}
														<div class="btn-group btn-group-sm" data-toggle="buttons">
															<label for="sendEmail" class="btn btn-sm btn-default {if $profile->notices == 'send email'}active{/if}"><input type="radio" value="send email" id="sendEmail" name="notices" {if $profile->notices == 'send email'}checked="checked"{/if}> {translate text="Send Email"}</label>
															<label for="dontSendEmail" class="btn btn-sm btn-default {if $profile->notices == 'do not send email'}active{/if}"><input type="radio" value="do not send email" id="dontSendEmail" name="notices" {if $profile->notices == 'do not send email'}checked="checked"{/if}> {translate text="Do not send email"}</label>
															<label for="optOut" class="btn btn-sm btn-default {if $profile->notices == 'opted out'}active{/if}"><input type="radio" value="opted out" id="optOut" name="notices" {if $profile->notices == 'opted out'}checked="checked"{/if}> {translate text="Opt-out"}</label>
														</div>
													{else}
														{$profile->notices}
													{/if}
											...
											<div class="form-group">
												...
												<div class="col-xs-8">
													...
													{else}
														{if $profile->emailReceiptFlag==0}{translate text="No"}{else}{translate text="Yes"}{/if}
													{/if}
												</div>
											</div>
														...
														<select name="phoneType" id="phoneType" class="form-control">
															{if count($phoneTypes) > 0}
																{foreach from=$phoneTypes item=phoneTypeLabel key=phoneType}
																	<option value="{$phoneType}" {if $phoneType == $profile->phoneType}selected="selected"{/if}>{$phoneTypeLabel|translate}</option>
																{/foreach}
															{else}
																<option></option>
															{/if}
											...
											<div class="form-group">
												...
												<div class="col-xs-8">
													...
													{else}
														{if $profile->availableHoldNotice==0}{translate text="No"}{else}{translate text="Yes"}{/if}
													{/if}
												</div>
											</div>
					...
					{if $allowAccountLinking}
						<div class="panel active">
							<a data-toggle="collapse" data-parent="#account-settings-accordion" href="#linkedAccountPanel">
								<div class="panel-heading">
									<div class="panel-title">
										{translate text="Linked Accounts"}
									</div>
								</div>
							</a>
