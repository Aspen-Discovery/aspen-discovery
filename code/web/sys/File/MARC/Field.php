...
class File_MARC_Field extends File_MARC_List {
	...
	function __construct($tag, $subfields = null, $ind1 = null, $ind2 = null) {
		...
		if (!preg_match("/^[0-9A-Za-z]{3}$/", $tag)) {
			$errorMessage = File_MARC_Exception::formatError(File_MARC_Exception::$messages[File_MARC_Exception::ERROR_INVALID_TAG], ["tag" => $tag]);
	...
	function formatField($exclude = ['2']) {
		if ($this->isControlField()) {
			return $this->getData();
		} else {
			$out = '';
			foreach ($this->getSubfields() as $subfield) {
				if (substr($this->getTag(), 0, 1) == '6' and (in_array($subfield->getCode(), [
						'v',
						'x',
						'y',
						'z',
