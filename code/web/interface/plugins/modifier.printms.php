...
function smarty_modifier_printms($ms) {
	...
	if ($hours) {
		...
		if ($days) {
			...
			if ($years) {
				return sprintf("%dyears %ddays %dhours %dminutes %dseconds", $years, $days, $hours, $minutes, $seconds);
			} else {
				return sprintf("%ddays %dhours %dminutes %dseconds", $days, $hours, $minutes, $seconds);
			}
		} else {
