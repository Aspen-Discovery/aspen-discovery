...
import { NativeBaseProvider, HStack, Spinner, Center } from "native-base";
...
// import helper files
import ThemeScreen from "./screens/Navigator";
...
class PermissionsScreen extends Component {
	...
	prepareResources = async () => {
		...
		if (Updates.releaseChannel == "production") {
			await AsyncStorage.setItem("releaseChannel", "production");
		} else if (Updates.releaseChannel == "beta") {
			this.setState({ isBeta: true });
			await AsyncStorage.setItem("releaseChannel", "beta");
		} else {
			await AsyncStorage.setItem("releaseChannel", "any");
		}
	};
