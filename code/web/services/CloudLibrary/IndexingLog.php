...
class CloudLibrary_IndexingLog extends Admin_Admin
{
	function launch()
	{
		global $interface;
		...
		$logEntries = array();
		$logEntry = new CloudLibraryExportLogEntry();
		...
		$options = array('totalItems' => $total,
		                 'fileName'   => $configArray['Site']['path'].'/CloudLibrary/IndexingLog?page=%d'. (empty($_REQUEST['pageSize']) ? '' : '&pageSize=' . $_REQUEST['pageSize']),
		                 'perPage'    => $pageSize,
		);
		$pager = new Pager($options);
		$interface->assign('pageLinks', $pager->getLinks());

		$this->display('cloudLibraryExportLog.tpl', 'Cloud Library Export Log');
	}
