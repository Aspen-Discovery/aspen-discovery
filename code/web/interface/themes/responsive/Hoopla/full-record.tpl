{include file="GroupedWork/load-full-record-view-enrichment.tpl"  recordType="hooplaRecord" recordId=$recordDriver->getId()}

{strip}
	<div class="full-record full-record-hoopla col-xs-12">
		{* Search Navigation *}
			...
			<div class="col-xs-4 col-sm-5 col-md-4 col-lg-3 text-center">
				{if $disableCoverArt != 1}
					<div id="recordCover" class="text-center row">
						<a href="#" onclick="return AspenDiscovery.Hoopla.getLargeCover('{$recordDriver->getUniqueID()}')"><img alt="{translate text='Book Cover' isPublicFacing=true inAttribute=true}" class="img-thumbnail{if $useOriginalCoverUrls} use-original-covers{/if} {$coverStyle}" src="{$recordDriver->getBookcoverUrl('medium')}" role="presentation"></a>
					</div>
				{/if}
						...
						<div class="btn-toolbar">
							<div class="btn-group btn-group-vertical btn-block">
								{foreach from=$actions item=curAction}
									...
									{else}
										<a href="#" class="btn btn-sm {if empty($curAction.btnType)}btn-action{else}{$curAction.btnType}{/if} btn-wrap" onclick="{$curAction.onclick}" {if !empty($curAction.alt)}title="{translate text=$curAction.alt inAttribute=true}"{/if} {if !empty($curAction['data-needs-refresh'])}data-needs-refresh="{$curAction['data-needs-refresh']}"{/if} {if !empty($curAction['data-record-id'])}data-record-id="{$curAction['data-record-id']}"{/if} {if !empty($curAction['data-record-source'])}data-record-source="{$curAction['data-record-source']}"{/if}>{translate text=$curAction.title isPublicFacing=true}</a>
									{/if}
								{/foreach}
							</div>
						</div>
