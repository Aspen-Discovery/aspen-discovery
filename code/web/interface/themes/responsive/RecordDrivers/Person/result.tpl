{strip}
<div id="record{$summId|escape}" class="resultsList-person resultsList">
	<div class="row">
		{if !empty($showCovers)}
		...
		<div class="{if empty($showCovers)}col-xs-12{else}col-xs-9 col-sm-9{if !empty($viewingCombinedResults)} col-md-9 col-lg-10{/if}{/if}">{* May turn out to be more than one situation to consider here *}
			...
			<div class="row">
				<div class="resultDetails col-md-9">
					...
					{/if}
					{if !empty($dateAdded)}
						<div class="result-date-added row">
					...
					{/if}
					{if !empty($lastUpdate)}
						<div class="result-last-update row">
